...
import com.duan.android.getuidemo.getui.GetuiSdkHttpPost;
import com.duan.android.getuidemo.utils.SoundUtils;
...
public class GetuiDemoActivity extends AppCompatActivity implements View.OnClickListener{

    private static final String TAG = "GetuiDemo";
    private static final String MASTERSECRET = "cWLhOUcCHL6KiV7quTjqo9";
    ...
    private Button btnPlayMusic;// 播放音频
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_getui_demo);
    ...
    public void onClick(View v) {
        if (v == clearBtn) {
            tLogView.setText("");
            DemoApplication.payloadData.delete(0, DemoApplication.payloadData.length());
        } else if (v == serviceBtn) {     // 点击“已启动/已停止”
            if (isServiceRunning) {
        ...
        } else if (v == bindAliasBtn) {    // 点击“绑定别名alias”
            bindAlias();
            // PushManager.getInstance().turnOnPush(this.getApplicationContext());
        } else if (v == unbindAliasBtn) {  // 点击“解绑别名-Alias”
            unBindAlias();
            // PushManager.getInstance().turnOffPush(this.getApplicationContext());
        } else if (v == transmissionBtn) { // 点击“透传测试”
            showTransmission();
        } else if (v == notificationBtn) {  // 点击“通知栏测试”
            showNotification();
        } else if (v == btnAddTag) {       // 点击“绑定标签-TAG”
            addTag();
        } else if (v == btnGetCid) {       // 点击“获取CID”
            getCid();
        } else if (v == btnSilentime) {    // 点击“静默时间”
            setSilentime();
        } else if (v == btnVersion) {      // 点击“版本号”
            getVersion();
        }else if (v == btnPlayMusic){   // 点击“播放音乐”
            playMusic(null); // 点击播放提示音
        }
    }
    ...
    private void initView() {
        ...
        tView = (TextView) findViewById(R.id.tvclientid);  // clientId
        ...
        tLogView = (EditText) findViewById(R.id.tvlog);    // 透传消息
        ...
        notificationBtn = (Button) findViewById(R.id.btn_psmsg);
    ...
    private void unBindAlias() {
        final EditText editText = new EditText(GetuiDemoActivity.this);
        new AlertDialog.Builder(GetuiDemoActivity.this).setTitle(R.string.unbind_alias).setView(editText)
                .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (alias.length() > 0) {
                            PushManager.getInstance().unBindAlias(GetuiDemoActivity.this, alias, false);
    ...
    private void bindAlias() {
        final EditText editText = new EditText(GetuiDemoActivity.this);
        new AlertDialog.Builder(GetuiDemoActivity.this).setTitle(R.string.bind_alias).setView(editText)
                .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (editText.getEditableText() != null) {
                            String alias = editText.getEditableText().toString();
                            if (alias.length() > 0) {
                                PushManager.getInstance().bindAlias(GetuiDemoActivity.this, alias);
    ...
    /**  播放声音 */
    private static String titleOrdered= "title_ordered";
    private static String titleQuote = "title_quote";
